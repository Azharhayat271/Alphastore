...
const NavBar = () => {
  ...
  const [headerColor, setHeaderColor] = useState('#ff471a'); // Default color
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5002/api/color/colors');
        setHeaderColor(response.data.headerColor);
        console.log(response.data.headerColor);
      } catch (error) {
        console.error('Error fetching header color:', error);
        // Handle the error, you might want to set a default color or show an error message
      }
    };
